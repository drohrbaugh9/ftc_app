...
public class CryptoboxDetector extends OpenCVPipeline {
    ...
    public Mat processFrame(Mat rgba, Mat gray) {
        ...
        //log("We have "+linesVertical+" vertical lines");
        ...
        double collumnLength =Lines.getPerpindicularDistance(left, right)/6;  // This assumes we see all the columns
        // need to adapt this for smaller field of view

        // Group vertical lines that fall within 1/6th of perp distance from leftmost to rightmost
        ...
        for (int i = 0; i < groupings.size()-1; i++) {
            ...
            if(count >= 10) {           // group together group i and group i+1 into one group
        ...
        for (Line line : columns) {
            ...
            if(rotateMat){
                Point rotPoint1 = new Point(-line.point1.y, line.point1.x);
                Point rotPoint2 = new Point(-line.point2.y, line.point2.x);
                Imgproc.line(rgba, rotPoint1, rotPoint2, new Scalar(20,165,240), 20);
            } else{
        ...
        for(int i=0;i<columns.size() - 1; i++) {
            ...
            if (rotateMat) {
                Imgproc.putText(rgba,"Col #" + i, new Point(-avgPoint.y, avgPoint.x - 15), 0, 1.5, new Scalar(0,255,255), 2);
                Imgproc.circle(rgba,new Point(-avgPoint.y,avgPoint.x), 15,new Scalar(0,255,0),6);
            }else{
