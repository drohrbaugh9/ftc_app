...
//@Disabled
public class BlueStandardCenter extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        driveMotors[0] = this.rightBack;
        driveMotors[1] = this.leftBack;
        driveMotors[2] = this.rightFront;
        driveMotors[3] = this.leftFront;

        // shooter motors
        this.shooter1 = Util.shooter1;
        this.shooter2 = Util.shooter2;

        shooterMotors = new DcMotor[2];
        shooterMotors[0] = this.shooter1;
        shooterMotors[1] = this.shooter2;
        ...
        offBeaconPower = AutoUtil.offBeaconPower;
        onBeaconPower = AutoUtil.onBeaconPower;
        ...
        do {
            ...
            double frontRedVal = I2C_ColorSensor.frontRedVal();
            Util.log("BEACON frontRed " + frontRedVal);
            double frontBlueVal = I2C_ColorSensor.frontBlueVal();
            Util.log("BEACON frontBlue " + frontBlueVal);
            double backRedVal = I2C_ColorSensor.backRedVal();
            Util.log("BEACON backRed " + backRedVal);
            double backBlueVal = I2C_ColorSensor.backBlueVal();
            Util.log("BEACON backBlue " + backBlueVal);
        ...
        Util.telemetry("frontRatio", frontRatio, false);
        Util.log("BEACON frontRatio " + frontRatio);
        Util.telemetry("backRatio", backRatio, false);
        Util.log("BEACON backRatio " + backRatio);
        Util.telemetry("frontRed", frontRed, false);
        Util.log("BEACON frontRed " + frontRed);
        Util.telemetry("backRed", backRed, true);
        Util.log("BEACON backRed " + backRed);

        boolean pressFront = false, pressBack = false;
        if ((frontRed == FALSE && backRed != FALSE) || (frontRed != TRUE && backRed == TRUE))
            pressFront = true;
        else if ((frontRed != FALSE && backRed == FALSE) || (frontRed == TRUE && backRed != TRUE))
            pressBack = true;
        ...
        if (!pressFront && !pressBack) {
            AutoUtil.encoderSteerBackward(BEACON_MOVE, 0.3, false);
            AutoUtil.beaconUp(upDown);
        } else if (pressFront) {
            AutoUtil.encoderSteerForward(BEACON_MOVE, offBeaconPower, true);
            AutoUtil.beaconDown(upDown);
        ...
        AutoUtil.encoderSteerBackward(2800, 0.1, 0.9, true);
